...
const fetchData = async () => {
    ...
    console.log("hi")
    console.log(response)
    state.hardwares = response.data.items
}
...
<template>
    <div class="container m-auto">
        <h1 class="text-5xl font-bold mb-6 mt-6">Hardwares</h1>
    </div>
    <div class="container m-auto">
        <div class="mb-6">
            <button class="rounded-lg h-[36px] bg-blue-500 hover:bg-blue-600 text-white text-center text-sm">
                <RouterLink class="mt-6 mb-6 mr-6 ml-6" :to="'/hardwares/add'">Add Hardware</RouterLink>
            </button>
        </div>
    </div>
        ...
        <section class="bg-blue-50 px-4 py-10">
            ...
            <table class="table-auto m-auto container border-separate border-spacing-2 border border-slate-400">
                <thead>
                    ...
                    <th class="border border-slate-300">Disks</th>
                    <th class="border border-slate-300">bmcRef</th>
                    <th class="border border-slate-300">interfaces</th>
                    <th class="border border-slate-300">Audit</th>
                    <th class="border border-slate-300">Delete</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="h in state.hardwares" :key="h.metadata.name">
                        <td class="border border-slate-300">{{ h.metadata.name }}</td>
                        <td class="border border-slate-300">{{ h.metadata.namespace }}</td>
                        <td class="border border-slate-300">{{ h.metadata.uid }}</td>
                        <td class="border border-slate-300">{{ h.spec.disks }}</td>
                        <td class="border border-slate-300">{{ h.spec.bmcRef }}</td>
                        <td class="border border-slate-300">{{ h.spec.interfaces }}</td>
                        <td class="border border-slate-300 h-[36px] bg-blue-500 hover:bg-blue-600 text-white text-center text-sm"><RouterLink :to="'/hardwares/'+h.metadata.name">View Details</RouterLink></td>
                        <td v-on:click="deleteMachine(h.metadata.name)" class="border border-slate-300 h-[36px] bg-red-500 hover:bg-red-600 text-white text-center text-sm">Delete</td>
                    </tr>
                </tbody>
            </table>
        </section>
