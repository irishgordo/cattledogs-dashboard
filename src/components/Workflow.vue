...
const fetchData = async () => {
    ...
    state.foundWorkflow = response.data
}
        ...
        <article class="bg-blue-50 pt-6 pb-6 pl-6 pr-6 space-y-4 container mt-6 ml-12">
            <section>
                <h3 class="text-left font-bold text-3xl">
                    {{ state.foundWorkflow.metadata.name }}
                </h3>
            </section>
            <section>
                <div class="sm:col-span-4">
                    ...
                    <div class="mt-2">
                        <span class="font-large">{{ state.foundWorkflow.metadata.namespace }}</span>
                    </div>
                </div>
            </section>
            <section class="space-y-6 m-auto container">
                <label class="block text-sm/6 font-xs text-red-900"><em>Spec:</em></label>
                <div class="md:col-span-4 whitespace-pre block bg-black-900">
                    {{ stringify(state.foundWorkflow.spec) }}
                </div>
            </section>
            <div class="mt-6 flex items-center justify-end gap-x-6">
                <RouterLink :to="'/workflows/'"><button type="button" class="text-sm/6 font-semibold text-gray-900">Back</button></RouterLink>
            </div>
        </article>
    </div>
